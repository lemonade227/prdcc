...
public:
  ...
  // Return the latest visible version (<= upper_bound) along with pointers.
  // For non-MVCC tables, version is always 0.
  virtual std::tuple<MetaDataType *, void *, uint64_t>
  search_latest(const void *key,
                uint64_t upper_bound = std::numeric_limits<uint64_t>::max()) = 0;
  ...
  virtual void for_each_key(std::function<void(const void *)> f) = 0;
};
...
public:
  ...
  std::tuple<MetaDataType *, void *, uint64_t>
  search_latest(const void *key,
                uint64_t upper_bound = std::numeric_limits<uint64_t>::max()) override {
    (void)upper_bound;
    auto [meta, value] = search(key);
    return std::make_tuple(meta, value, 0);
  }
  ...
  void for_each_key(std::function<void(const void *)> f) override {
    map_.for_each([&](const KeyType &key,
                      const std::tuple<MetaDataType, ValueType> &value) {
      f(&key);
    });
  }
...
public:
  ...
  search_latest(const void *key,
    ...
    uint64_t version_exclusive = upper_bound;
    if (upper_bound != std::numeric_limits<uint64_t>::max()) {
      version_exclusive = upper_bound + 1;
    }

    uint64_t found_version = 0;
    std::tuple<MetaDataType, ValueType> *row_ptr = nullptr;
    bool ok = map_.get_key_version_prev_with_version(k, version_exclusive,
                                                     found_version, row_ptr);
    CHECK(ok) << "key does not exist.";
    return std::make_tuple(&std::get<0>(*row_ptr), &std::get<1>(*row_ptr),
                           found_version);
  }
  ...
  void for_each_key(std::function<void(const void *)> f) override {
    map_.for_each_key([&](const KeyType &key) { f(&key); });
  }
