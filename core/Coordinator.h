...
public:
  ...
  void start() {
    ...
    // run benchmark_time seconds
    auto timeToRun = static_cast<int>(context.benchmark_time);
    auto warmup = static_cast<int>(context.benchmark_warmup);
    auto cooldown = static_cast<int>(context.benchmark_cooldown);
    CHECK(timeToRun > 0);
    CHECK(warmup >= 0);
    CHECK(cooldown >= 0);
    CHECK(warmup + cooldown < timeToRun)
        << "warmup + cooldown must be smaller than benchmark_time.";
    ...
    uint64_t total_commit = 0, total_abort_no_retry = 0, total_abort_lock = 0,
             ...
             total_sql_parse_us = 0, total_sql_plan_us = 0,
             total_sql_exec_us = 0, total_sql_queue_wait_us = 0,
             total_sql_cache_hit = 0, total_sql_cache_miss = 0,
             total_sql_txn = 0;
    int count = 0;
    ...
    do {
      ...
      uint64_t n_commit = 0, n_abort_no_retry = 0, n_abort_lock = 0,
               ...
               n_sql_parse_us = 0, n_sql_plan_us = 0, n_sql_exec_us = 0,
               n_sql_queue_wait_us = 0, n_sql_cache_hit = 0,
               n_sql_cache_miss = 0, n_sql_txn = 0;
      ...
      for (auto i = 0u; i < workers.size(); i++) {
        ...
        n_sql_parse_us += workers[i]->sql_metrics.parse_us.load();
        workers[i]->sql_metrics.parse_us.store(0);
        n_sql_plan_us += workers[i]->sql_metrics.plan_us.load();
        workers[i]->sql_metrics.plan_us.store(0);
        n_sql_exec_us += workers[i]->sql_metrics.exec_us.load();
        workers[i]->sql_metrics.exec_us.store(0);
        n_sql_queue_wait_us += workers[i]->sql_metrics.queue_wait_us.load();
        workers[i]->sql_metrics.queue_wait_us.store(0);
        n_sql_cache_hit += workers[i]->sql_metrics.cache_hit.load();
        workers[i]->sql_metrics.cache_hit.store(0);
        n_sql_cache_miss += workers[i]->sql_metrics.cache_miss.load();
        workers[i]->sql_metrics.cache_miss.store(0);
        n_sql_txn += workers[i]->sql_metrics.txn_total.load();
        workers[i]->sql_metrics.txn_total.store(0);
      }
      ...
      if (count > warmup && count <= timeToRun - cooldown) {
        ...
        total_sql_parse_us += n_sql_parse_us;
        total_sql_plan_us += n_sql_plan_us;
        total_sql_exec_us += n_sql_exec_us;
        total_sql_queue_wait_us += n_sql_queue_wait_us;
        total_sql_cache_hit += n_sql_cache_hit;
        total_sql_cache_miss += n_sql_cache_miss;
        total_sql_txn += n_sql_txn;
      }
    ...
    if (context.enable_sql_emulator) {
      LOG(INFO) << "average sql per second: txns "
                << 1.0 * total_sql_txn / count
                << ", parse/plan/exec/queue us "
                << 1.0 * total_sql_parse_us / count << "/"
                << 1.0 * total_sql_plan_us / count << "/"
                << 1.0 * total_sql_exec_us / count << "/"
                << 1.0 * total_sql_queue_wait_us / count
                << ", cache hit ratio "
                << (total_sql_cache_hit + total_sql_cache_miss == 0
                        ? 0.0
                        : 100.0 * total_sql_cache_hit /
                              (total_sql_cache_hit + total_sql_cache_miss))
                << "%";
    }
