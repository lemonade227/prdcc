...
public:
  ...
  // Return the value of a particular key and the version older than the
  // specific version (exclusive). Returns false if not exists.
  bool get_key_version_prev_with_version(const KeyType &key, uint64_t version,
                                        uint64_t &out_version,
                                        ValueType *&out_value) {
    return apply(
        [&key, version, &out_version, &out_value](HashMapType &map) -> bool {
          auto it = map.find(key);
          if (it == map.end()) {
            return false;
          }
          ...
          for (VersionTupleType &vt : l) {
            if (get_version(vt) < version) {
              out_version = get_version(vt);
              out_value = &get_value(vt);
              return true;
            }
          }
          return false;
  ...
  void for_each_key(std::function<void(const KeyType &)> f) {
