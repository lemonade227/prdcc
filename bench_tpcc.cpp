...
DEFINE_string(query, "neworder", "tpcc query, mixed, neworder, payment, delivery");
DEFINE_int32(neworder_dist, 10, "new order distributed.");
DEFINE_int32(payment_dist, 15, "payment distributed.");
DEFINE_int32(delivery_dist, 1, "delivery distributed.");
...
int main(int argc, char *argv[]) {
  ...
  } else if (FLAGS_query == "delivery") {
    context.workloadType = aria::tpcc::TPCCWorkloadType::DELIVERY_ONLY;
  } else {
    CHECK(false);
  }
  ...
  context.deliveryCrossPartitionProbability = FLAGS_delivery_dist;
