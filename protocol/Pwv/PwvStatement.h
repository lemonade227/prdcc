...
public:
  ...
  void execute() override {
    ...
    int32_t D_NEXT_O_ID = query.O_ID;
...
public:
  ...
  void execute() override {
    ...
    // write to district
    {
      auto tableId = tpcc::district::tableID;
    ...
    auto newOrderTableID = tpcc::new_order::tableID;
    db.find_table(newOrderTableID, W_ID - 1)
        ->update(&storage.new_order_key, &storage.new_order_value);
    ...
    auto orderTableID = tpcc::order::tableID;
    db.find_table(orderTableID, W_ID - 1)
        ->update(&storage.order_key, &storage.order_value);
    ...
    for (int i = 0; i < query.O_OL_CNT; i++) {
      ...
      db.find_table(orderLineTableID, W_ID - 1)
          ->update(&storage.order_line_keys[i], &storage.order_line_values[i]);
    }
  }
...
class PwvDeliveryStatement : public PwvStatement {
public:
  PwvDeliveryStatement(tpcc::Database &db, const tpcc::Context &context,
                       tpcc::Random &random, tpcc::Storage &storage,
                       std::size_t partition_id,
                       const tpcc::DeliveryQuery &query)
      : db(db), context(context), random(random), storage(storage),
        partition_id(partition_id), query(query) {}

  ~PwvDeliveryStatement() override = default;

  void prepare_read_and_write_set() override {
    int32_t W_ID = query.W_ID;

    for (int32_t D_ID = 1; D_ID <= context.n_district; D_ID++) {
      int32_t district_idx = D_ID - 1;
      int32_t O_ID = query.NO_O_ID[district_idx];
      if (O_ID == 0) {
        continue;
      }

      storage.delivery_new_order_keys[district_idx] =
          tpcc::new_order::key(W_ID, D_ID, O_ID);
      PwvRWKey new_order_key;
      new_order_key.set_table_id(tpcc::new_order::tableID);
      new_order_key.set_partition_id(W_ID - 1);
      new_order_key.set_key(&storage.delivery_new_order_keys[district_idx]);
      new_order_key.set_value(&storage.delivery_new_order_values[district_idx]);
      readSet.push_back(new_order_key);
      writeSet.push_back(new_order_key);

      storage.delivery_order_keys[district_idx] =
          tpcc::order::key(W_ID, D_ID, O_ID);
      PwvRWKey order_key;
      order_key.set_table_id(tpcc::order::tableID);
      order_key.set_partition_id(W_ID - 1);
      order_key.set_key(&storage.delivery_order_keys[district_idx]);
      order_key.set_value(&storage.delivery_order_values[district_idx]);
      readSet.push_back(order_key);
      writeSet.push_back(order_key);

      for (int32_t OL_NUMBER = 1; OL_NUMBER <= query.O_OL_CNT[district_idx];
           OL_NUMBER++) {
        storage.delivery_order_line_keys[district_idx][OL_NUMBER - 1] =
            tpcc::order_line::key(W_ID, D_ID, O_ID, OL_NUMBER);
        PwvRWKey order_line_key;
        order_line_key.set_table_id(tpcc::order_line::tableID);
        order_line_key.set_partition_id(W_ID - 1);
        order_line_key.set_key(
            &storage.delivery_order_line_keys[district_idx][OL_NUMBER - 1]);
        order_line_key.set_value(
            &storage.delivery_order_line_values[district_idx][OL_NUMBER - 1]);
        readSet.push_back(order_line_key);
        writeSet.push_back(order_line_key);
      }

      storage.delivery_customer_keys[district_idx] =
          tpcc::customer::key(W_ID, D_ID, query.O_C_ID[district_idx]);
      PwvRWKey customer_key;
      customer_key.set_table_id(tpcc::customer::tableID);
      customer_key.set_partition_id(W_ID - 1);
      customer_key.set_key(&storage.delivery_customer_keys[district_idx]);
      customer_key.set_value(&storage.delivery_customer_values[district_idx]);
      readSet.push_back(customer_key);
      writeSet.push_back(customer_key);
    }
  }

  std::size_t piece_partition_id() override { return query.W_ID - 1; }

  void execute() override {
    int32_t W_ID = query.W_ID;
    uint64_t delivery_date = Time::now();
    ...
    for (int32_t D_ID = 1; D_ID <= context.n_district; D_ID++) {
      ...
      auto new_order_table =
          db.find_table(tpcc::new_order::tableID, W_ID - 1);
      auto new_order_row = new_order_table->search(
          &storage.delivery_new_order_keys[district_idx]);
      PwvHelper::read(new_order_row,
                      &storage.delivery_new_order_values[district_idx],
                      new_order_table->value_size());

      auto order_table = db.find_table(tpcc::order::tableID, W_ID - 1);
      auto order_row =
          order_table->search(&storage.delivery_order_keys[district_idx]);
      PwvHelper::read(order_row, &storage.delivery_order_values[district_idx],
                      order_table->value_size());

      int32_t O_OL_CNT = query.O_OL_CNT[district_idx];
      if (storage.delivery_new_order_values[district_idx].NO_DUMMY != 0 ||
          storage.delivery_order_values[district_idx].O_CARRIER_ID != 0) {
        continue;
      }

      float total_amount = 0;
      auto order_line_table =
          db.find_table(tpcc::order_line::tableID, W_ID - 1);
      for (int32_t OL_NUMBER = 1; OL_NUMBER <= O_OL_CNT; OL_NUMBER++) {
        auto &order_line_key =
            storage.delivery_order_line_keys[district_idx][OL_NUMBER - 1];
        auto &order_line_value =
            storage.delivery_order_line_values[district_idx][OL_NUMBER - 1];
        auto order_line_row = order_line_table->search(&order_line_key);
        PwvHelper::read(order_line_row, &order_line_value,
                        order_line_table->value_size());
        order_line_value.OL_DELIVERY_D = delivery_date;
        total_amount += order_line_value.OL_AMOUNT;
        order_line_table->update(&order_line_key, &order_line_value);
      }

      auto customer_table = db.find_table(tpcc::customer::tableID, W_ID - 1);
      auto customer_row =
          customer_table->search(&storage.delivery_customer_keys[district_idx]);
      PwvHelper::read(customer_row,
                      &storage.delivery_customer_values[district_idx],
                      customer_table->value_size());

      storage.delivery_new_order_values[district_idx].NO_DUMMY = 1;
      new_order_table->update(&storage.delivery_new_order_keys[district_idx],
                              &storage.delivery_new_order_values[district_idx]);

      storage.delivery_order_values[district_idx].O_CARRIER_ID =
          query.O_CARRIER_ID;
      order_table->update(&storage.delivery_order_keys[district_idx],
                          &storage.delivery_order_values[district_idx]);

      storage.delivery_customer_values[district_idx].C_BALANCE += total_amount;
      storage.delivery_customer_values[district_idx].C_DELIVERY_CNT += 1;
      customer_table->update(&storage.delivery_customer_keys[district_idx],
                             &storage.delivery_customer_values[district_idx]);
    }
  }
