    ...
    if (context.coordinator_num == 1) {
      ...
      if (!init_transaction) {
        for (auto i = id; i < transactions.size(); i += context.worker_num) {
          ...
          setupHandlers(*transactions[i]);
          auto prepared_sql_state = transactions[i]->sql_state;
          transactions[i]->reset();
          // The initial prepare phase already paid SQL frontend once while
          // building the read/write set. The subsequent Aria execution should
          // reuse that frontend state rather than being treated as a retry or
          // paying it twice.
          transactions[i]->sql_state = prepared_sql_state;
          transactions[i]->setup_process_requests_in_execution_phase();
        }
      } else {
        ...
        for (auto i = id; i < transactions.size(); i += context.worker_num) {
          transactions[i]->reset();
          // same-batch reuse should re-enable frontend execution for the new
          // epoch, but it is not a retry attempt.
          sql::on_transaction_reuse(transactions[i]->sql_state);
          transactions[i]->setup_process_requests_in_execution_phase();
