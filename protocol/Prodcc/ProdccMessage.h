/**
 * @file ProdccMessage.h
 * @brief Network message definitions for PRDCC.
 *
 * This module defines PRDCC request/response message types and the factories
 * and handlers used for remote reads, reservation, validation checks, writes,
 * and metadata cleanup across coordinators.
 */
#pragma once
...
public:
  static std::size_t new_search_message(Message &message, ITable &table,
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(ProdccMessage::SEARCH_REQUEST), message_size,
  ...
  static std::size_t new_reserve_message(Message &message, ITable &table,
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(ProdccMessage::RESERVE_REQUEST), message_size,
        table.tableID(), table.partitionID());
  ...
  static std::size_t new_check_message(Message &message, ITable &table,
    ...
    auto message_size = MessagePiece::get_header_size() + key_size +
                        ...
                        sizeof(bool);
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(ProdccMessage::CHECK_REQUEST), message_size,
        table.tableID(), table.partitionID());
  ...
  static std::size_t new_write_message(Message &message, ITable &table,
    ...
    auto message_size = MessagePiece::get_header_size() + key_size + field_size +
                        sizeof(uint32_t) + sizeof(uint32_t);
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(ProdccMessage::WRITE_REQUEST), message_size,
        table.tableID(), table.partitionID());
    ...
    encoder << tid << tid_offset;
    message.flush();
    return message_size;
  }

  static std::size_t new_cleanup_message(Message &message, ITable &table,
                                         uint32_t tid, uint32_t tid_offset,
                                         const void *key) {
    auto key_size = table.key_size();
    ...
    auto message_size = MessagePiece::get_header_size() + key_size +
                        sizeof(uint32_t) + sizeof(uint32_t);
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(ProdccMessage::CLEANUP_REQUEST), message_size,
        table.tableID(), table.partitionID());
...
class ProdccMessageHandler {
  using Transaction = ProdccTransaction;
...
public:
  ...
  search_request_handler(MessagePiece inputPiece, Message &responseMessage,
    ...
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(ProdccMessage::SEARCH_REQUEST));
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(ProdccMessage::SEARCH_RESPONSE), message_size,
  ...
  search_response_handler(MessagePiece inputPiece, Message &responseMessage,
    ...
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(ProdccMessage::SEARCH_RESPONSE));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  reserve_request_handler(MessagePiece inputPiece, Message &responseMessage,
    ...
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(ProdccMessage::RESERVE_REQUEST));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  check_request_handler(MessagePiece inputPiece, Message &responseMessage,
    ...
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(ProdccMessage::CHECK_REQUEST));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
    ...
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(ProdccMessage::CHECK_RESPONSE), message_size,
        table_id, partition_id);
  ...
  check_response_handler(MessagePiece inputPiece, Message &responseMessage,
    ...
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(ProdccMessage::CHECK_RESPONSE));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  write_request_handler(MessagePiece inputPiece, Message &responseMessage,
                        ...
                        std::vector<std::unique_ptr<Transaction>> &txns) {
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(ProdccMessage::WRITE_REQUEST));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
    ...
    auto message_size = MessagePiece::get_header_size() + sizeof(tid) +
                        sizeof(tid_offset);
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(ProdccMessage::WRITE_RESPONSE), message_size,
        table_id, partition_id);
  ...
  static void
  write_response_handler(MessagePiece inputPiece, Message &responseMessage,
                         ITable &table,
                         std::vector<std::unique_ptr<Transaction>> &txns) {
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(ProdccMessage::WRITE_RESPONSE));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  static void
  cleanup_request_handler(MessagePiece inputPiece, Message &responseMessage,
                          ITable &table,
                          std::vector<std::unique_ptr<Transaction>> &txns) {
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(ProdccMessage::CLEANUP_REQUEST));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
    ...
    auto message_size = MessagePiece::get_header_size() + sizeof(tid) +
                        sizeof(tid_offset);
    auto message_piece_header = MessagePiece::construct_message_piece_header(
        static_cast<uint32_t>(ProdccMessage::CLEANUP_RESPONSE), message_size,
        table_id, partition_id);
  ...
  static void
  cleanup_response_handler(MessagePiece inputPiece, Message &responseMessage,
                           ITable &table,
                           std::vector<std::unique_ptr<Transaction>> &txns) {
    DCHECK(inputPiece.get_message_type() ==
           static_cast<uint32_t>(ProdccMessage::CLEANUP_RESPONSE));
    auto table_id = inputPiece.get_table_id();
    auto partition_id = inputPiece.get_partition_id();
    DCHECK(table_id == table.tableID());
    DCHECK(partition_id == table.partitionID());
  ...
  get_message_handlers() {
    ...
    v.push_back(cleanup_request_handler);
    v.push_back(cleanup_response_handler);
    return v;
  }
};
