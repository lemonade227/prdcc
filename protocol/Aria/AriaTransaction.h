...
public:
  ...
  AriaTransaction(std::size_t coordinator_id, std::size_t partition_id,
      ...
      : coordinator_id(coordinator_id), partition_id(partition_id),
        ...
        attemptStartTime(startTime), partitioner(partitioner) {
    reset();
  }
...
public:
  ...
  // Per-epoch attempt start time (used for optional timer breakdown).
  std::chrono::steady_clock::time_point attemptStartTime;
