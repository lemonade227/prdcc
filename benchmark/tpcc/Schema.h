...
#define ORDER_VALUE_FIELDS(x, y)                                               \
  x(int32_t, O_C_ID) y(uint64_t, O_ENTRY_D) y(int32_t, O_CARRIER_ID)             \
...
public:
  std::string operator()(const tpcc::customer::value &v) {
    return Serializer<decltype(v.C_DATA)>()(v.C_DATA) +
           ...
           Serializer<decltype(v.C_DELIVERY_CNT)>()(v.C_DELIVERY_CNT);
  }
};
...
public:
  std::size_t operator()(StringPiece str, tpcc::customer::value &result) const {
    ...
    str.remove_prefix(sz_payment_cnt);
    std::size_t sz_delivery_cnt =
        Deserializer<decltype(result.C_DELIVERY_CNT)>()(
            str, result.C_DELIVERY_CNT);
...
public:
  static constexpr std::size_t size() {
    return ClassOf<decltype(tpcc::customer::value::C_DATA)>::size() +
           ...
           ClassOf<decltype(tpcc::customer::value::C_DELIVERY_CNT)>::size();
  }
};
