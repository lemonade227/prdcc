...
public:
  ...
  NewOrder(std::size_t coordinator_id, std::size_t partition_id,
      ...
      : Transaction(coordinator_id, partition_id, partitioner), db(db),
        ...
        query(makeNewOrderQuery()(context, partition_id + 1, random)) {resolve_new_order_query();query.populate_ranges();}

  virtual ~NewOrder() override = default;

  TransactionResult execute(std::size_t worker_id) override {
    sql::run_frontend_once_lazy(context, this->sql_state, [this]() {
    ...
    this->update(newOrderTableID, W_ID - 1, storage.new_order_key,
                 storage.new_order_value);
    ...
    this->update(orderTableID, W_ID - 1, storage.order_key,
                 storage.order_value);
    ...
    for (int i = 0; i < query.O_OL_CNT; i++) {
      ...
      this->update(orderLineTableID, W_ID - 1, storage.order_line_keys[i],
                   storage.order_line_values[i]);
    }
  ...
  void reset_query() override {
    ...
    resolve_new_order_query();
    query.populate_ranges();
  }

  void get_query_ranges(std::vector<std::pair<uint64_t, uint64_t>>& read_ranges_out,
                        std::vector<std::pair<uint64_t, uint64_t>>& write_ranges_out) const {
      read_ranges_out = this->query.read_ranges;
      write_ranges_out = this->query.write_ranges;
  }

private:
  void resolve_new_order_query() {
    int32_t W_ID = partition_id + 1;
    district::key district_key(W_ID, query.D_ID);
    auto *district_value = static_cast<district::value *>(std::get<1>(
        db.find_table(district::tableID, W_ID - 1)->search_latest(&district_key)));
...
public:
  ...
  Payment(std::size_t coordinator_id, std::size_t partition_id,
      ...
      : Transaction(coordinator_id, partition_id, partitioner), db(db),
        context(context), random(random), storage(storage),
        partition_id(partition_id),
        query(makePaymentQuery()(context, partition_id + 1, random)) {query.populate_ranges(context);}
  ...
  TransactionResult execute(std::size_t worker_id) override {
    ...
    if (context.operation_replication) {
      ...
      encoder << storage.customer_value.C_BALANCE
              ...
              << storage.customer_value.C_DELIVERY_CNT;
    }
...
public:
  ...
  Delivery(std::size_t coordinator_id, std::size_t partition_id,
           DatabaseType &db, const ContextType &context, RandomType &random,
           Partitioner &partitioner, Storage &storage)
      : Transaction(coordinator_id, partition_id, partitioner), db(db),
        context(context), random(random), storage(storage),
    ...
    resolve_delivery_query();
    query.populate_ranges(context);
  }

  virtual ~Delivery() override = default;
  ...
  TransactionResult execute(std::size_t worker_id) override {
    ...
    for (int32_t D_ID = 1; D_ID <= context.n_district; D_ID++) {
      ...
      if (should_skip_delivery_district(district_idx)) {
        continue;
      }
      int32_t O_ID = query.NO_O_ID[district_idx];

      storage.delivery_new_order_keys[district_idx] =
          new_order::key(W_ID, D_ID, O_ID);
      this->search_for_update(newOrderTableID, W_ID - 1,
                              storage.delivery_new_order_keys[district_idx],
                              storage.delivery_new_order_values[district_idx]);

      storage.delivery_order_keys[district_idx] = order::key(W_ID, D_ID, O_ID);
      this->search_for_update(orderTableID, W_ID - 1,
    ...
    for (int32_t D_ID = 1; D_ID <= context.n_district; D_ID++) {
      ...
      for (int32_t OL_NUMBER = 1; OL_NUMBER <= O_OL_CNT; OL_NUMBER++) {
        storage.delivery_order_line_keys[district_idx][OL_NUMBER - 1] =
            order_line::key(W_ID, D_ID, O_ID, OL_NUMBER);
        this->search_for_update(
            orderLineTableID, W_ID - 1,
            storage.delivery_order_line_keys[district_idx][OL_NUMBER - 1],
    ...
    for (int32_t D_ID = 1; D_ID <= context.n_district; D_ID++) {
      ...
      int32_t C_ID = query.O_C_ID[district_idx];
      storage.delivery_customer_keys[district_idx] =
          customer::key(W_ID, D_ID, C_ID);
      this->search_for_update(customerTableID, W_ID - 1,
                              storage.delivery_customer_keys[district_idx],
    ...
    for (int32_t D_ID = 1; D_ID <= context.n_district; D_ID++) {
      ...
      this->update(newOrderTableID, W_ID - 1,
                   storage.delivery_new_order_keys[district_idx],
                   storage.delivery_new_order_values[district_idx]);

      storage.delivery_order_values[district_idx].O_CARRIER_ID =
          query.O_CARRIER_ID;
      this->update(orderTableID, W_ID - 1,
                   storage.delivery_order_keys[district_idx],
                   storage.delivery_order_values[district_idx]);
      ...
      for (int32_t OL_NUMBER = 1; OL_NUMBER <= O_OL_CNT; OL_NUMBER++) {
        ...
        this->update(orderLineTableID, W_ID - 1, order_line_key,
                     order_line_value);
      }

      if (this->execution_phase) {
        storage.delivery_customer_values[district_idx].C_BALANCE +=
            total_amount;
        storage.delivery_customer_values[district_idx].C_DELIVERY_CNT += 1;
      }
      this->update(customerTableID, W_ID - 1,
                   storage.delivery_customer_keys[district_idx],
                   storage.delivery_customer_values[district_idx]);
    }
  ...
  void get_query_ranges(
      std::vector<std::pair<uint64_t, uint64_t>> &read_ranges_out,
      std::vector<std::pair<uint64_t, uint64_t>> &write_ranges_out) const {
    read_ranges_out = this->query.read_ranges;
    write_ranges_out = this->query.write_ranges;
  }
...
private:
  bool should_skip_delivery_district(int32_t district_idx) const {
    return query.NO_O_ID[district_idx] == 0 ||
           (this->execution_phase &&
            !is_delivery_candidate_available(district_idx));
  }

  bool is_delivery_candidate_available(int32_t district_idx) const {
    return storage.delivery_new_order_values[district_idx].NO_DUMMY == 0 &&
           storage.delivery_order_values[district_idx].O_CARRIER_ID == 0;
  }

  void resolve_delivery_query() {
    int32_t W_ID = query.W_ID;
    for (int32_t D_ID = 1; D_ID <= context.n_district; D_ID++) {
