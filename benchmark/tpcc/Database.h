...
public:
  ...
  bool find_oldest_new_order(int32_t W_ID, int32_t D_ID, int32_t &O_ID,
                             int32_t &C_ID, int32_t &O_OL_CNT) {
    ITable *new_order_table = find_table(new_order::tableID, W_ID - 1);
    ITable *order_table = find_table(order::tableID, W_ID - 1);

    district::key d_key(W_ID, D_ID);
    auto *d_value = static_cast<district::value *>(std::get<1>(
        find_table(district::tableID, W_ID - 1)->search_latest(&d_key)));

    int32_t next_hint = oldest_new_order_hint(W_ID, D_ID);
    for (int32_t candidate_o_id = next_hint;
         candidate_o_id < d_value->D_NEXT_O_ID; candidate_o_id++) {
      new_order::key no_key(W_ID, D_ID, candidate_o_id);
      if (!new_order_table->contains(&no_key)) {
        next_hint = candidate_o_id + 1;
        continue;
      }
      auto *no_value = static_cast<new_order::value *>(
          std::get<1>(new_order_table->search_latest(&no_key)));
      if (no_value->NO_DUMMY != 0) {
        next_hint = candidate_o_id + 1;
        continue;
      }

      order::key o_key(W_ID, D_ID, candidate_o_id);
      if (!order_table->contains(&o_key)) {
        next_hint = candidate_o_id + 1;
        continue;
      }
      auto *o_value = static_cast<order::value *>(
          std::get<1>(order_table->search_latest(&o_key)));
      if (o_value->O_CARRIER_ID != 0) {
        next_hint = candidate_o_id + 1;
        continue;
      }

      advance_oldest_new_order_hint(W_ID, D_ID, next_hint);
      O_ID = candidate_o_id;
      C_ID = o_value->O_C_ID;
      O_OL_CNT = o_value->O_OL_CNT;
      return true;
    }

    advance_oldest_new_order_hint(W_ID, D_ID, d_value->D_NEXT_O_ID);
    O_ID = 0;
    C_ID = 0;
    O_OL_CNT = 0;
    return false;
  ...
  void initialize(const Context &context) {
    KeyConverter::configure_tpcc(context.partition_num, context.n_district);
    key_domain_min_ = KeyConverter::tpcc_min_key();
    key_domain_max_ = KeyConverter::tpcc_max_key();
    ...
    oldest_new_order_hints.clear();
    for (auto partitionID = 0u; partitionID < partitionNum; partitionID++) {
      oldest_new_order_hints.emplace_back(new std::atomic<int32_t>[10]);
      for (int32_t D_ID = 1; D_ID <= 10; D_ID++) {
        oldest_new_order_hints.back()[D_ID - 1].store(2101);
      }
    ...
    initTables(
        ...
        partitionNum, threadsNum, partitioner.get());
    if (context.workloadType == TPCCWorkloadType::DELIVERY_ONLY ||
        context.workloadType == TPCCWorkloadType::MIXED) {
    ...
    if (!context.mvcc) {
      LOG(INFO)
          << "Pre-computing and caching all TPC-C keys for DataSummary...";
      this->all_keys_cache_ = collect_all_keys_internal();
      LOG(INFO) << "Key caching complete. Cached "
                << this->all_keys_cache_.size() << " keys.";
    }
  }
    ...
    if (is_neworder) {
...

      {
        ...
        dec >> customer_value.C_BALANCE >> customer_value.C_YTD_PAYMENT >>
            customer_value.C_PAYMENT_CNT >> customer_value.C_DELIVERY_CNT;
      }
    }
  }

  const std::vector<uint64_t>& get_all_keys() const {
      DCHECK(!all_keys_cache_.empty()) << "Key cache is empty. Was Database::initialize called?";
      return all_keys_cache_;
  }

  std::pair<uint64_t, uint64_t> get_key_domain() const {
      return {key_domain_min_, key_domain_max_};
  }

  bool get_key_domain_is_dense() const {
      return true;
  }

private:
  int32_t oldest_new_order_hint(int32_t W_ID, int32_t D_ID) const {
    if (oldest_new_order_hints.empty()) {
      return 2101;
    }
    return oldest_new_order_hints[W_ID - 1][D_ID - 1].load(
        std::memory_order_relaxed);
  }

  void advance_oldest_new_order_hint(int32_t W_ID, int32_t D_ID,
                                     int32_t O_ID) {
    if (oldest_new_order_hints.empty()) {
      return;
    }
    auto &hint = oldest_new_order_hints[W_ID - 1][D_ID - 1];
    int32_t current = hint.load(std::memory_order_relaxed);
    while (current < O_ID &&
           !hint.compare_exchange_weak(current, O_ID, std::memory_order_relaxed,
                                       std::memory_order_relaxed)) {
    }
  }

  std::vector<uint64_t> collect_all_keys_internal() {
      std::vector<uint64_t> all_keys;
      std::mutex m;

      LOG(INFO) << "Starting to collect all TPC-C keys for data summary...";

      auto collect_from_table_vec = 
          [&](const std::vector<std::unique_ptr<ITable>>& table_vec, auto key_converter) {
          using KeyType = std::remove_pointer_t<typename aria::Argument0<decltype(key_converter)>>;
          for (const auto& table_ptr : table_vec) {
              table_ptr->for_each_key([&](const void* key_ptr) {
                  const KeyType* k = static_cast<const KeyType*>(key_ptr);
                  std::lock_guard<std::mutex> lock(m);
                  all_keys.push_back(KeyConverter::to_uint64(*k));
              });
          }
      };
    
      collect_from_table_vec(tbl_warehouse_vec, [](warehouse::key*){});
      collect_from_table_vec(tbl_district_vec, [](district::key*){});
      collect_from_table_vec(tbl_customer_vec, [](customer::key*){});
      collect_from_table_vec(tbl_customer_name_idx_vec, [](customer_name_idx::key*){});
      collect_from_table_vec(tbl_new_order_vec, [](new_order::key*){});
      collect_from_table_vec(tbl_order_vec, [](order::key*){});
      collect_from_table_vec(tbl_order_line_vec, [](order_line::key*){});
      collect_from_table_vec(tbl_item_vec, [](item::key*){});
      collect_from_table_vec(tbl_stock_vec, [](stock::key*){});
    
      LOG(INFO) << "Collected " << all_keys.size() << " TPC-C keys.";
      return all_keys;
  }
...
private:
  ...
  std::vector<std::unique_ptr<std::atomic<int32_t>[]>> oldest_new_order_hints;
  std::vector<uint64_t> all_keys_cache_;
  uint64_t key_domain_min_ = 0;
  uint64_t key_domain_max_ = 0;
};
} // namespace tpcc
} // namespace aria
