...
public:
  ...
  void initialize(const Context &context) {
    ...
    const std::size_t total_keys =
        context.keysPerPartition * context.partition_num;
    key_domain_max_ =
        total_keys == 0 ? 0 : static_cast<uint64_t>(total_keys - 1);
    ...
    if (!context.mvcc) {
      LOG(INFO)
          << "Pre-computing and caching all YCSB keys for DataSummary...";
...
private:
  std::vector<uint64_t> collect_all_keys_internal() {
      ...
      LOG(INFO) << "Starting to collect all YCSB keys for data summary...";
      for (const auto& table_ptr : tbl_ycsb_vec) {
          table_ptr->for_each_key([&](const void* key_ptr) {
              const ycsb::key* k = static_cast<const ycsb::key*>(key_ptr);
              std::lock_guard<std::mutex> lock(m);
              all_keys.push_back(KeyConverter::to_uint64(*k));
          });
      }
      LOG(INFO) << "Collected " << all_keys.size() << " YCSB keys.";
      return all_keys;
  }
